...
**Key points:**\
     ...
     - for the entity, write a repository class (e.g., for the `Product` entity write `ProductNaturalRepository`) that extends the `NaturalRepositoryImpl` and use it for setting the entity class type and the natural id type (when multiple natural IDs are used, the type is not relevant anymore, simply set it to `Serializable`)\
