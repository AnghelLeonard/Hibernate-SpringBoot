...
74. **[How To Optimize Batch Inserts in Parent-Child Relationships With Cascade Persist](https://github.com/AnghelLeonard/Hibernate-SpringBoot/tree/master/HibernateSpringBootWindowFunctionPaging)**

**Description:** Let's suppose that we have a one-to-many relationship between `Tournament` and `TennisPlayer` entities. We want to create create a bunch of tournaments with players and save them in the database (e.g., a MySQL database) using the batch technique. By default, this will result in batching the players only. In order to batch the tournaments as well, we need to order inserts as in this application.
